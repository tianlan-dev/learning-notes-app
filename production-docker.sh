...
image_name() {
  printf '%s\n' "${DOCKER_IMAGE_NAME:-learning-notes-app:local}"
}

container_name() {
  printf '%s\n' "${DOCKER_CONTAINER_NAME:-learning-notes-app}"
}
...
start_container() {
  ...
  local notes_source="${LEARNING_NOTES_SOURCE:-$SCRIPT_DIR/learning-notes}"
  if [[ ! -e "$notes_source" ]]; then
    echo "Notes source not found: $notes_source" >&2
    exit 1
  fi
  ...
  echo "Log: $log_file"
  {
    ...
    echo "Action: $action"
    docker build \
      --build-context "learning_notes_source=$notes_source" \
      -t "$image" \
      "$SCRIPT_DIR"
